...
function getRootPath() {
 ...
 $list = $("#fileList"),
    ...
    var uploader = WebUploader.create({
		auto:true,
		swf: getRootPath()+'/js/webuploader-0.1.5/Uploader.swf',
	
		// 文件接收服务端。
		server: getRootPath()+'/upload',
	...
	uploader.on( 'fileQueued', function( file ) {
		var $li = $(
			'<div id="' + file.id + '" class="item">' +
				...
				'<div class="info" id="filename">' + file.name + '</div>' +
	...
	uploader.on( 'uploadSuccess', function( file , response) {
		...
		if ($("#image").length<1)
		{
		   var image  = "<input type='hidden' id='image' name='img' value='"+fileName+"'/>";
